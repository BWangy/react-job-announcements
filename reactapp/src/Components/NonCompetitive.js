...
const NonCompetitive = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await fetch("https://localhost:7207/api/NonCompetitive");
                if (!response.ok) {
    ...
    const columns = [
        { title: "Title", field: "title", render: rowData => (<Link to={`/ApplyNonCompetitive?id=${rowData.identifier}`}> {rowData.title} </Link>) },
    ...
    return (
        <div className="container p-5" style={{ maxWidth: "100%" }}>
            ...
            <ThemeProvider theme={defaultMaterialTheme}>
                <MaterialTable columns={columns} data={jobs} title="Non-Competitive Jobs" />
            </ThemeProvider>
        </div>
    );
};

export default NonCompetitive;
